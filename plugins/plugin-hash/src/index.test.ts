...
describe("hash", () => {
  ...
  test("base 16 (hex)", async () => {
    const image = await Jimp.fromBuffer(
      await fs.readFile(getTestImagePath("rgb.tiff"))
    );
  ...
  test("base 64", async () => {
    const image = await Jimp.fromBuffer(
      await fs.readFile(getTestImagePath("windows95.bmp"))
    );
  ...
  test("base 23", async function () {
    const image = await Jimp.fromBuffer(
      await fs.readFile(getTestImagePath("panoramic.jpg"))
    );
...
describe("pHash", () => {
  test("should calculate the distance using distanceFromHash", async function () {
    const image1 = await Jimp.fromBuffer(
      await fs.readFile(getTestImagePath("dice.png"))
    );
    const image2 = await Jimp.fromBuffer(
      await fs.readFile(getTestImagePath("mask.png"))
    );
